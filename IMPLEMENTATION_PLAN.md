...
**Files:** `packages/codec/src/{index.ts,frame.ts,parse.ts,avl.ts,codec12.ts,crc16.ts}`, `packages/codec/dictionaries/*.json` (34 tables, GENERATED by `tools/avl-dict` from 105 Teltonika model pages — the four hand-made family files are gone), `packages/codec/__fixtures__/{wiki/*.hex.json,traccar/*.hex.json,captures/*.bin}`, `tools/redact/index.ts`.
...
**Approach:** profiles seeded per MODEL (105 of them, generated from the AVL catalogue; the four original family rows are kept as `legacy` because live devices reference them, and hidden from the picker) with presence_rules & command_presets JSON; CSV import = upload → dry-run diff (create/update/error rows) → confirm → apply; registry sync (Redis `registry:imei`) on create/retire.
