...
describe('E03-2 meta-test: manifest completeness (AC[3])', () => {
  it('every registered /v1 data route has a manifest entry — an unlisted route fails the suite', () => {
    ...
    const EXEMPT = /^\/(healthz|metrics)$|^\/v1\/(auth|ws-ticket|devices\/last|profiles|branding|internal\/caddy-ask|public\/pilot-request|public\/signup|public\/share|driver-scores|routing|stream|reports|api-keys|billing|webhooks|push|partner|openapi\.json|docs)(?:\/|$)|^\/v1\/\*$/
