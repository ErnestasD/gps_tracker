...
async function seedTenant(db: Db, poolInsertEvent: (t: string, a: string, dev: string) => Promise<string>, name: string): Promise<TenantFixture> {
...
export async function setup(): Promise<Fixtures> {
  ...
  const t1 = await seedTenant(db, insertEvent, 'T1')
  const t2 = await seedTenant(db, insertEvent, 'T2')
