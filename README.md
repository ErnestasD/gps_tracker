...
| `packages/kb` | the knowledge base ("Learn") — articles in EN/LT/PL/DE, rendered by BOTH the public site and the dashboard. Zero runtime deps; two entry points (root = metadata, `/content` = the bodies). See ADR-042 |
...
| `SES_SNS_TOPIC_ARN` | apps/api | **Required for the bounce/complaint feed.** The one SNS topic `POST /v1/webhooks/ses` accepts messages from (`arn:aws:sns:<region>:<acct>:<topic>`). A valid AWS signature only proves *AWS* sent the message — the same regional certificate signs every AWS customer's topics — so without this binding a stranger's topic could suppress any address platform-wide (audit C2). **Fails closed:** unset ⇒ every message is refused 403, including the subscription handshake, so set it **before** deploying or the SNS subscription stays `PendingConfirmation` forever |
| `SES_ADMIN_ACCESS_KEY_ID` / `SES_ADMIN_SECRET_ACCESS_KEY` | apps/api + apps/worker | **Separate from the SMTP credentials above**, which can send but cannot manage identities. An IAM user limited to `ses:CreateEmailIdentity`, `ses:GetEmailIdentity` and `ses:DeleteEmailIdentity`, used to register a white-label tenant's own sending domain (ADR-036). Needs `AWS_REGION` too. Absent ⇒ the four `/v1/tenant/sending-domain` routes answer 503 and the branding card reports the feature unavailable; every tenant's mail keeps going out on `MAIL_FROM`, exactly as before |
| `AWS_REGION` | apps/api + apps/worker | region for the SES identity calls above — **the same region as `SMTP_HOST`** (`eu-central-1` today). Identities are per-region, so pointing this anywhere else finds an empty account: every verify answers 502 and no tenant can ever finish setup. Only read together with the two `SES_ADMIN_*` keys |
...
| `STRIPE_INCLUDED` | apps/worker | `basePriceId:count,…` — included device count per TSP plan; the daily reporter bills devices beyond it. **This is the only place plan allowances live** (300/1,000/3,500) |
| `STRIPE_TAX_ENABLED` | apps/api | `1` turns on Stripe Tax at checkout (VAT on top, EU B2B reverse charge, VAT-ID collection). **Off** — MB Dokigo is not VAT-registered yet; prices already carry `tax_behavior: exclusive` so the switch is this flag plus a head office in Stripe. See `docs/runbooks/stripe-catalog.md` |
...
| `PLATFORM_DOMAIN` | apps/api, **apps/worker** | our own domain (`orbetra.com`). Lets a tenant claim `<slug>.<domain>` as a white-label host with NO DNS work — created already verified, since we hold the zone. **Requires a `*.<domain>` A record**; unset ⇒ the option is not offered and every domain goes through DNS TXT. The **worker** needs it too: it ranks a tenant's OWN domain above their `<slug>.<domain>` one when choosing the host their mail points at. Unset there ⇒ the old oldest-verified rule, under which the zero-setup subdomain is always oldest and wins forever (audit W-5) |
...
`/login` (a chooser — the site never authenticates tenant users), `/demo`, `/docs`, `/learn` +
`/learn/<slug>` (the knowledge base — ADR-042), `/cookies`,
...
| Surface | Route | Brand |
|---|---|---|
| public site | `/learn`, `/learn/<slug>` | ours; indexable, per-article `<title>`/description |
| dashboard | `/app/learn`, `/app/learn/<slug>` | the tenant's — same articles, under their name |

**Why it is a package and not a docs site.** The dashboard is white-labelled: a "read the docs" link
in a reseller's product is our brand on their domain, one click from their customer. So the in-app
help is the help itself, not a link out. The DNS panel's `↗` used to be exactly such a link; it now
opens the same explanation in-product, at the same anchors.

How the one source stays safe on both surfaces:

- Prose says `{product}` — the site substitutes ours, the app substitutes the tenant's (or a neutral
...
- `packages/kb`'s own tests enforce it: every article in every language, anchors identical across
  ...
  route — and **no app-visible article naming the platform**.

**Two entry points, deliberately.** The root (`@orbetra/kb`) carries types, categories, slugs, the
gate and `KB_META` (titles + audience, ~43 KB of source) — cheap enough to import anywhere. The bodies live
behind `@orbetra/kb/content` and are imported only by the two lazily-loaded help routes, so a page
carrying six help links ships no prose, and an operator who never opens the help never downloads it.
`KB_META` is generated by `pnpm --filter @orbetra/kb gen:meta` and asserted equal to the articles
by the tests; `apps/web/__tests__/kbUiLabels.spec.ts` additionally holds every bolded UI label in the
articles to the word the product's own locale uses.
...
channels, "Never reported", DNS records, the reporting-interval sliders, CAN parameters, trip
distance, the account time zone, the data export, the billing usage panel).
