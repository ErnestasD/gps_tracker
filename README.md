...
| `LOCKOUT_MAX_FAILS_PER_IP` | apps/api | **Soft** per-IP ceiling on FAILED logins in the window, default `50`. Refuses **before** argon2 for a source no successful login has ever come from; for a source one has (a 24 h marker), it only throttles — a shared egress like an office NAT or carrier CGNAT is never denied a valid credential. Decays by one per success |
