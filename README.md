...
| `BILLING_GRACE_DAYS` | apps/worker | days after a subscription lapses (or a trial expires) before the daily sweep counts the tenant as actionable in `billing_lapsed_actionable`; default `14`. The sweep only COUNTS — it never suspends anyone |
