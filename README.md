...
| `SES_ADMIN_ACCESS_KEY_ID` / `SES_ADMIN_SECRET_ACCESS_KEY` | apps/api | **Separate from the SMTP credentials above**, which can send but cannot manage identities. An IAM user limited to `ses:CreateEmailIdentity`, `ses:GetEmailIdentity` and `ses:DeleteEmailIdentity`, used to register a white-label tenant's own sending domain (ADR-036). Needs `AWS_REGION` too. Absent ⇒ the four `/v1/tenant/sending-domain` routes answer 503 and the branding card reports the feature unavailable; every tenant's mail keeps going out on `MAIL_FROM`, exactly as before |
| `AWS_REGION` | apps/api | region for the SES identity calls above — **the same region as `SMTP_HOST`** (`eu-central-1` today). Identities are per-region, so pointing this anywhere else finds an empty account: every verify answers 502 and no tenant can ever finish setup. Only read together with the two `SES_ADMIN_*` keys |
