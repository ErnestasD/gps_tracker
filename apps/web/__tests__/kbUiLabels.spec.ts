...
const BANNED: Partial<Record<KbLang, Banned[]>> = {
  lt: [
    ...
    { wrong: /\bAtvėsimo laik\w*/i, right: 'Pertrauka', key: 'rules.cooldown', why: 'this is the rule field on screen' },
    { wrong: /\bperžaidim\w*/i, right: 'atkūrim', key: 'playback.title', why: 'the screen has a Lithuanian name' },
    { wrong: /\bplanin\w+ ataskait\w+/i, right: 'Suplanuot', key: 'scheduled.title', why: 'this is what the reports screen is called' },
    { wrong: /(?<!auto)\bpark(as|o|e|ą|ui|ų|ai|ams|uose)\b/i, right: 'Autoparkas', key: 'shell.fleet', why: 'a bare "parkas" is a green park; the nav item spells it out' },
    { wrong: /\bplanuokl(is|io|iui|į|iu|yje)\b/i, right: 'planuoklė', key: 'routing.title', why: 'the screen uses the feminine form' },
  ],
  pl: [
    { wrong: /\bkomend[aęąyi]?\b/i, right: 'Polecenie', key: 'devices.cmd.command', why: 'every screen says this' },
