...
/**
 * The marketing site's background, ported verbatim from apps/site/src/components/site/
 * OrbitalFluidBg.tsx so the dashboard's sign-in screen IS the partner sign-in rather than a
 * lookalike. Self-contained: no imports beyond React, no site tokens.
 *
 * ONE change from the original — the brand colour is read from `--accent` at mount instead of
 * being the hard-coded `76,77,207`. On our hosts that resolves to the product accent and the
 * canvas is pixel-identical; on a reseller's host `applyBranding` has already set it to theirs, so
 * the animation is in THEIR colour. A second hard-coded purple here would have re-opened the leak
 * the branding work just closed, on the largest surface of the page.
 */
function accentRgb(): { r: number; g: number; b: number } {
  const fallback = { r: 76, g: 77, b: 207 }; // the site's own brand blue
...
export function OrbitalFluidBg({ className = "" }: OrbitalFluidBgProps) {
  ...
  useEffect(() => {
    ...
    if (!ctx) return;
    const A = accentRgb();
    const acc = (a: number | string) => `rgba(${A.r},${A.g},${A.b},${a})`;
    ...
    const draw = () => {
      ...
      for (const ring of rings) {
        ...
        ctx.restore();
        ctx.strokeStyle = acc(ring.opacity);
        ctx.lineWidth = 0.9;
        ctx.stroke();
        ...
        satGrad.addColorStop(0.3, acc(0.5));
        satGrad.addColorStop(1, acc(0));
      ...
      for (let i = comets.length - 1; i >= 0; i--) {
        const c = comets[i];
        if (c === undefined) continue; // noUncheckedIndexedAccess: apps/web is stricter than apps/site
        c.life += dt;
        if (c.life > c.maxLife) {
          comets.splice(i, 1);
          continue;
