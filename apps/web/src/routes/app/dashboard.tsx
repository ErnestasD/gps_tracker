...
import { getReadiness } from '@/lib/branding'
 ...
 *
 * Without it the gate is a silent 403 on the Create-account button, which is a worse product than
...
function ReadinessCard() {
  const { t } = useTranslation()
  const navigate = useNavigate()
  const r = useQuery({ queryKey: ['readiness'], queryFn: getReadiness, staleTime: 60_000 })
  const data = r.data
  if (data === undefined || data.mode === 'not_white_label') return null
