...
export interface WorkerStrings {
  ...
  /** `brand` may be EMPTY: the recipient lookup can fail and the report still sends (audit W-8).
   *  An empty brand must render a report belonging to nobody — never to the platform. */
...
const EN: WorkerStrings = {
  ...
  reportSubject: (brand, title, from, to) => (brand === '' ? `${title} report (${from} to ${to})` : `${brand} — ${title} report (${from} to ${to})`),
  reportFooter: (tz, brand) => (brand === '' ? `All times shown in ${tz}.` : `All times shown in ${tz}. Generated by ${brand}.`),
...
const LT: WorkerStrings = {
  ...
  reportSubject: (brand, title, from, to) => (brand === '' ? `Ataskaita: ${title} (${from}–${to})` : `${brand} — ataskaita: ${title} (${from}–${to})`),
  reportFooter: (tz, brand) => (brand === '' ? `Visas laikas rodomas ${tz} zona.` : `Visas laikas rodomas ${tz} zona. Parengė ${brand}.`),
...
const DE: WorkerStrings = {
  ...
  reportSubject: (brand, title, from, to) => (brand === '' ? `Bericht: ${title} (${from} – ${to})` : `${brand} — Bericht: ${title} (${from} – ${to})`),
  reportFooter: (tz, brand) => (brand === '' ? `Alle Zeiten in ${tz}.` : `Alle Zeiten in ${tz}. Erstellt von ${brand}.`),
...
const PL: WorkerStrings = {
  ...
  reportSubject: (brand, title, from, to) => (brand === '' ? `Raport: ${title} (${from} – ${to})` : `${brand} — raport: ${title} (${from} – ${to})`),
  reportFooter: (tz, brand) => (brand === '' ? `Wszystkie godziny w strefie ${tz}.` : `Wszystkie godziny w strefie ${tz}. Wygenerowano przez ${brand}.`),
