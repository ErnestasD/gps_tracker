...
function renderReportHtml(
  ...
  try {
    ...
    return renderBrandedEmail(opts.branding ?? {}, tenantName, { subject, bodyHtml, locale: opts.locale }, { whiteLabel: opts.whiteLabel })
  } catch {
    return undefined
  }
}
...
interface RecipientContext {
  ...
  locale: string | undefined
  units: DisplayUnits
  /** the tenant PLAN says they are a reseller (audit W-4); `undefined` = plan unreadable, so the
   *  renderer falls back to the older branding-key heuristic rather than claiming the tenant */
  whiteLabel: boolean | undefined
}

// `brand: ''` and not 'Orbetra' (audit W-8). This default is reached when the recipient lookup
// THROWS — a schema skew, a connection fault — and the report still goes out to addresses the
// customer's own account_manager typed in. Substituting our name there put `Orbetra — Mileage
// report` in a reseller's customer's inbox and `Generated by Orbetra.` in the footer, on a failure
// path nothing reports. An empty brand degrades to a report with no vendor on it, which is the
// honest outcome: we could not establish whose report it is, so we claim it for nobody.
const DEFAULT_RECIPIENT: RecipientContext = { brand: '', branding: undefined, tenantName: undefined, locale: undefined, units: METRIC_UNITS, whiteLabel: false }
...
async function resolveRecipient(pool: Pool, tenantId: string, accountId: string, appBaseUrl?: string): Promise<RecipientContext> {
  try {
    ...
    return { brand, branding, tenantName, locale: row.locale ?? undefined, units: sanitizeUnits(row), whiteLabel: whiteLabelFromPlan(row.plan) }
  } catch (err) {
