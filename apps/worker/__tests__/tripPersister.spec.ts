...
const closeEv = (deviceId: bigint): CloseEvent => ({
  type: 'close', deviceId, startTime: new Date(1_000), endTime: new Date(9_000),
  startLat: 54, startLon: 25, endLat: 54.1, endLon: 25.1, distanceM: 1234, distanceSource: 'gps', maxSpeed: 88, idleS: 60,
})
...
/** Fake redis registry: device:tenant / device:account hashes. */
function fakeRedis(map: Record<string, { t: string; a: string }>) {
  return {
