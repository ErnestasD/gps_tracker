...
export function AdminButton({
  ...
  const base =
    "inline-flex items-center justify-center gap-2 rounded-md font-medium transition-all cursor-pointer disabled:cursor-not-allowed disabled:opacity-50 whitespace-nowrap outline-none focus-visible:ring-2 focus-visible:ring-[var(--admin-brand)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--admin-surface)]";
  ...
  return <button className={cn(base, sz, className)} style={style} {...props} />;
}
...
export function StatCard({
  ...
  return (
    <div className="admin-card flex flex-col gap-2 p-3 sm:p-4">
      <div className="flex items-start justify-between gap-2">
        ...
        {delta && (
          <Badge
            tone={delta.tone === "up" ? "success" : delta.tone === "down" ? "danger" : "neutral"}
      ...
      <div
        className="display text-xl font-semibold tracking-tight sm:text-2xl"
...
function Sparkline({ data }: { data: number[] }) {
  ...
  return (
    <svg
      ...
      className="h-6 w-14 shrink-0 sm:w-20"
      aria-hidden
    >
      <polyline
        fill="none"
...
export function AdminInput({ className, ...props }: React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      className={cn(
        "h-9 w-full rounded-md border px-3 text-sm outline-none transition-colors placeholder:opacity-60 focus:ring-2 focus:ring-[var(--admin-brand)]/30",
        className,
      )}
      style={{
...
export function AdminLabel({ className, ...props }: React.LabelHTMLAttributes<HTMLLabelElement>) {
  return (
    <label
      className={cn("mb-1 block text-xs font-medium", className)}
      style={{ color: "var(--admin-ink-soft)" }}
      {...props}
    />
  );
...
export function AdminSwitch({
  ...
  return (
    <label className={cn("inline-flex cursor-pointer items-center gap-2", disabled && "opacity-50")}>
      <button
        type="button"
        role="switch"
      ...
      {label && <span className="text-sm" style={{ color: "var(--admin-ink)" }}>{label}</span>}
    </label>
  );
}
