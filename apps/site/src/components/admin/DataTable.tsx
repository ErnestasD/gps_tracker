    ...
    if (q && searchable) {
      ...
      out = out.filter((row) => {
        ...
        return Object.values(row as Record<string, unknown>).some((v) => {
          ...
          return s.toLowerCase().includes(l);
        });
      });
    }
      ...
      <div className="admin-hairline-b flex flex-wrap items-center gap-2 p-3">
        ...
        {anyFilterActive && (
          <button
            ...
            className="h-9 cursor-pointer rounded-md border px-3 text-sm"
                ...
                >
                  {c.sortable ? (
                    <button
                      ...
                      className="inline-flex cursor-pointer items-center gap-1 hover:text-[var(--admin-ink)]"
                    >
                      {c.header}
                      {sort?.key === c.key ? (
