 ...
 *
 * A copy of apps/web's `lib/sheet.ts`, deliberately. The demo cannot import from the dashboard —
 * they are separate apps with separate builds — and this file is forty lines of arithmetic whose
 * whole point is that the demo behaves exactly as the product does. The shared packages hold zod
 * schemas and DB access, not view geometry; adding one for this would be a bigger commitment than
 * the duplication. If the product's sheet changes, this changes with it.
 */
...
export const SHEET_STORAGE_KEY = 'orbetra.demoInspectorSheetPx'
