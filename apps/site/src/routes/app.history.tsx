...
function HistoryPage() {
  ...
  const [pt, setPt] = React.useState<{ x: number; y: number }>({ x: 60, y: 300 });
  ...
  React.useEffect(() => {
    ...
    const p = path.getPointAtLength((pos / 199) * len);
    setPt({ x: p.x, y: p.y });
  }, [pos]);
      ...
      <div className="admin-card overflow-hidden">
        ...
        <div className="relative h-[360px]" style={{ background: "var(--admin-surface-sunken)" }}>
          <svg viewBox="0 0 1000 360" className="h-full w-full">
            ...
            />
            <circle cx={pt.x} cy={pt.y} r="8" fill="var(--admin-brand)" stroke="#fff" strokeWidth="3" />
          ...
          <div className="mb-3 flex items-center justify-between gap-3">
            <div className="flex items-center gap-1">
              ...
              <AdminButton size="sm" onClick={() => setPlaying((p) => !p)}>
                {playing ? <Pause className="h-4 w-4" /> : <Play className="h-4 w-4" />} {playing ? "Pauzė" : "Leisti"}
              </AdminButton>
