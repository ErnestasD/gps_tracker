...
`sonner` (inline success/error patterns are testid-pinned), `@radix-ui/react-dialog`/`react-tabs`
(inline forms + trivial hand-rolled tabs), `tw-animate-css`, `framer-motion` (marketing-only).
