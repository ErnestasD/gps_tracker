...
types, categories, slugs, the gating predicate and `KB_META` — metadata and titles, ~43 KB of
source. The
...
`KB_META` is generated by `packages/kb/scripts/gen-meta.ts` (`pnpm --filter @orbetra/kb gen:meta`)
and asserted identical to the articles — slug order, category, surfaces, audience, screen, and every
language's title and summary — by the package's own tests. A generated file nobody checks is a file
that goes stale; the script is the fix and the test is the gate.
...
- The public site gains `/learn` and `/learn/$slug`; the article chunk is ~600 KB raw / ~230 KB
  ...
  entry chunk grows only by `KB_META` (~43 KB raw), which the home page's band and every help link
  read.
...
- **Articles quote the product's own words, and that is now tested.** `apps/web/__tests__/kbUiLabels.spec.ts`
  is a registry of words the product does not use, each paired with the i18n key holding the word it
  does — so the replacement is read from the locale and a product rename breaks the test rather than
  silently making the articles right again. It exists because the first draft of this content was
  written from schemas and memory instead of from the locale files: the Lithuanian corpus called the
  hardware `seklys` (a sleuth) 112 times where the product says `sekiklis`, and the article whose
  whole purpose is the status vocabulary had four of its six labels wrong. Structural tests could not
  see any of it — every sentence was well-formed and every anchor resolved.
