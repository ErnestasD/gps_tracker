...
export const Route = createFileRoute("/pricing")({
  head: () => ({
    meta: [
      { title: "Pricing — Orbetra GPS for small fleets" },
      { name: "description", content: "Flat per-vehicle pricing for GPS tracking. From €6/vehicle/month. 30-day free trial. No credit card, cancel anytime." },
    ],
  }),
...
const COMPARE: { row: string; s: string | boolean; g: string | boolean; x: string | boolean }[] = [
  { row: "Vehicles", s: "1–3", g: "4–20", x: "20+" },
  { row: "Price per vehicle / mo", s: "€9", g: "€7", x: "€6" },
  { row: "Live map & trip history", s: true, g: true, x: true },
  { row: "Idle & speeding alerts", s: true, g: true, x: true },
  { row: "Unlimited users & drivers", s: false, g: true, x: true },
  { row: "Geofences & SMS alerts", s: false, g: true, x: true },
  { row: "PDF & Excel reports", s: false, g: true, x: true },
  { row: "REST API & webhooks", s: false, g: false, x: true },
  { row: "Custom roles & permissions", s: false, g: false, x: true },
  { row: "99.9% uptime SLA", s: false, g: false, x: true },
  { row: "Support", s: "Email", g: "Priority", x: "Account manager" },
];
    ...
    <>
      <section className="mx-auto max-w-7xl px-6 pt-20 md:pt-28 pb-10 text-center">
        <span className="section-label justify-center">
          ...
          — PRICING · PER VEHICLE · EXCL. VAT
        </span>
        <h1 className="display text-5xl md:text-6xl font-bold leading-[1.02] mt-6 text-ink">
          One flat price.<br />
          <span className="text-gradient">Per vehicle. Per month.</span>
        </h1>
        <p className="mt-6 text-lg text-muted-foreground max-w-2xl mx-auto">
          No seat licences, no setup fees, no long contracts. Add a vehicle when you add a van — same price, same day.
        </p>
      </section>

      <section className="px-6 pb-8">
        <div className="mx-auto max-w-6xl surface-card p-4 flex flex-wrap items-center gap-3 justify-center">
          <span className="mono text-[11px] tracking-[0.2em] uppercase text-[var(--brand-amber)]">— FREE TRIAL</span>
          <span className="text-sm text-ink/85">30 days free, up to 5 vehicles. No credit card. Cancel anytime.</span>
        </div>
      </section>
      ...
      <section className="px-6 py-20">
        <div className="mx-auto max-w-6xl">
          ...
          <p className="mt-6 mono text-xs text-muted-foreground text-center">
            All prices exclude VAT. Billed monthly per vehicle. Change plans or cancel any time — data always exports.
          </p>
        </div>
      </section>
    </>
