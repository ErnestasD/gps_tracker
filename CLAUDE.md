...
13. **Free-stack mandate.** Maps = MapLibre + OpenFreeMap style URL (env `TILES_STYLE_URL`); reverse geocoding = our self-hosted Photon (`GEOCODER_URL`) through the geocode-cache service. Never introduce Google Maps, Mapbox, or any paid geo API. OSM attribution stays visible on every map view.
